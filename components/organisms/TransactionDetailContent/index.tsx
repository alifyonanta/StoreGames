          ...
          <div className="main-content main-content-card overflow-auto">
            <section className="checkout mx-auto">
                ...
                <div className="game-checkout d-flex flex-row align-items-center">
                  <div className="pe-4">
                    <div className="cropped">
                      <img
                        src={`/img/${data.historyVoucherTopup.thumbnail}.png`}
                        width="200"
                        height="130"
                        className="img-fluid"
                        alt=""
              ...
              <div className="purchase pt-30">
                ...
                <Row label="Price" value={`Rp ${(data.historyVoucherTopup.price).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`} />
                <Row label="Tax 10%" value={`Rp ${(data.tax).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`} />
                <Row label="Total" value={`Rp ${(data.value).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`} className="color-palette-4"/>
              </div>
